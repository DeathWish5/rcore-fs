...
pub struct AsyncDevFS {
    devs: RwLock<BTreeMap<String, Arc<dyn AsyncINode>>>,
    self_ref: Weak<AsyncDevFS>,
}
...
#[async_trait]
impl AsyncFileSystem for AsyncDevFS {
    async fn sync(&self) -> Result<()> {
        Ok(())
    }

    async fn root_inode(&self) -> Arc<dyn AsyncINode> {
        Arc::new(AsyncDevRootINode {
            fs: self.self_ref.upgrade().unwrap(),
        })
    }
...
impl AsyncDevFS {
    ...
    pub fn add(&self, name: &str, dev: Arc<dyn AsyncINode>) -> Result<()> {
        let mut devs = self.devs.write();
        if devs.contains_key(name) {
            return Err(FsError::EntryExist);
        }
...
#[async_trait]
impl AsyncINode for AsyncDevRootINode {
    ...
    async fn create(&self, _name: &str, _type_: FileType, _mode: u32) -> Result<Arc<dyn AsyncINode>> {
        Err(FsError::NotSupported)
    }
