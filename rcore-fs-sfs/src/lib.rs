...
trait DeviceExt: Device {
    async fn read_block(&self, id: BlockId, offset: usize, buf: &mut [u8]) -> vfs::Result<()> {
        ...
        match self.read_at(id * BLKSIZE + offset, buf).await {
            ...
            Ok(len) => panic!(
                "read invalid len {}, expected len {} block {} offset {}",
                len,
                buf.len(),
                id,
                offset
    ...
    async fn write_block(&self, id: BlockId, offset: usize, buf: &[u8]) -> vfs::Result<()> {
        ...
        match self.write_at(id * BLKSIZE + offset, buf).await {
            Ok(len) if len == buf.len() => Ok(()),
            Ok(len) => panic!(
                "write invalid len {}, expected len {}, block {} offset {}",
                len,
                buf.len(),
                id,
                offset
...
impl INodeImpl {
    ...
    async fn set_disk_block_id(
        &self,
        file_block_id: BlockId,
        disk_block_id: BlockId,
    ) -> vfs::Result<()> {
        match file_block_id {
        ...
        for mut range in iter {
            ...
            self.fs
                ...
                .read_block(
                    range.block,
                    range.begin,
                    &mut buf[buf_offset..buf_offset + range.len()],
                )
                .await?;
            buf_offset += range.len();
        }
        ...
        for mut range in iter {
            ...
            self.fs
                ...
                .write_block(
                    range.block,
                    range.begin,
                    &buf[buf_offset..buf_offset + range.len()],
                )
                .await?;
            buf_offset += range.len();
        }
    ...
    async fn _clean_at(&self, begin: usize, end: usize) -> vfs::Result<usize> {
        ...
        for mut range in iter {
            ...
            self.fs
                .device
                .write_block(range.block, range.begin, &ZEROS[..range.len()])
                .await?;
            buf_offset += range.len();
        }
        Ok(buf_offset)
