...
#[async_trait]
trait AsyncDeviceExt: AsyncDevice {
...
impl AsyncDeviceExt for dyn AsyncDevice {}
...
/// INode for SFS
pub struct AsyncINodeImpl {
    /// INode number
    id: INodeId,
    /// On-disk INode
    disk_inode: RwLock<Dirty<DiskINode>>,
    /// Reference to SFS, used by almost all operations
    fs: Arc<AsyncSimpleFileSystem>,
    /// Char/block device id (major, minor)
    /// e.g. crw-rw-rw- 1 root wheel 3, 2 May 13 16:40 /dev/null
    device_inode_id: usize,
}

impl Debug for AsyncINodeImpl {
    fn fmt(&self, f: &mut Formatter) -> Result<(), Error> {
        write!(
            f,
            "AsyncINode {{ id: {}, disk: {:?} }}",
...
impl AsyncINodeImpl {
    ...
    //     fn init(device: &'a Arc<dyn AsyncDevice>, range: &'a BlockRange, offset: usize);
    // }
    ...
    // where 
    // F: FnMut(&Arc<dyn AsyncDevice>, &BlockRange, usize) -> Box<Pin<dyn Future<Output = vfs::Result<()>>>>,
    // {
    //     let size = self.disk_inode.read().size as usize;
    //     let iter = BlockIter {
    //         begin: size.min(begin),
    ...
    async fn _read_at(&self, offset: usize, buf: &mut [u8]) -> vfs::Result<usize> {
        ...
        // Box::new(async move |device: &Arc<dyn AsyncDevice>, range: &BlockRange, offset: usize| {
    ...
    async fn _write_at(&self, offset: usize, buf: &[u8]) -> vfs::Result<usize> {
        ...
        //     async move |device: &Arc<dyn AsyncDevice>, range: &BlockRange, offset: usize| {
    ...
    pub async fn link_inodeimpl(&self, name: &str, other: &Arc<AsyncINodeImpl>) -> vfs::Result<()> {
        let info = self.metadata()?;
        if info.type_ != vfs::FileType::Dir {
            return Err(FsError::NotDir);
        }
...
#[async_trait]
impl vfs::AsyncINode for AsyncINodeImpl {
    ...
    async fn create2(
        ...
        _mode: u32,
        data: usize,
    ) -> vfs::Result<Arc<dyn vfs::AsyncINode>> {
        let info = self.metadata()?;
        if info.type_ != vfs::FileType::Dir {
            return Err(FsError::NotDir);
        }
    ...
    async fn link(&self, name: &str, other: &Arc<dyn AsyncINode>) -> vfs::Result<()> {
        let info = self.metadata()?;
        if info.type_ != vfs::FileType::Dir {
            return Err(FsError::NotDir);
        }
    ...
    async fn move_(&self, old_name: &str, target: &Arc<dyn AsyncINode>, new_name: &str) -> vfs::Result<()> {
        let info = self.metadata()?;
        if info.type_ != vfs::FileType::Dir {
            return Err(FsError::NotDir);
        }
    ...
    async fn find(&self, name: &str) -> vfs::Result<Arc<dyn vfs::AsyncINode>> {
        let info = self.metadata()?;
        if info.type_ != vfs::FileType::Dir {
            return Err(FsError::NotDir);
        }
    ...
    fn fs(&self) -> Arc<dyn vfs::AsyncFileSystem> {
        self.fs.clone()
    }
    fn as_any_ref(&self) -> &dyn Any {
        self
...
/// 将SFS设置为内部可变，即对外接口全部是&self，struct的全部field用RwLock包起来
/// 这样其内部各field均可独立访问
pub struct AsyncSimpleFileSystem {
    /// on-disk superblock
    super_block: RwLock<Dirty<SuperBlock>>,
    /// blocks in use are mared 0
    free_map: RwLock<Dirty<BitVec<Lsb0, u8>>>,
    /// inode list
    inodes: RwLock<BTreeMap<INodeId, Weak<AsyncINodeImpl>>>,
    /// device
    device: Arc<dyn AsyncDevice>,
    /// Pointer to self, used by INodes
    self_ptr: Weak<AsyncSimpleFileSystem>,
    /// device inode
    device_inodes: RwLock<BTreeMap<usize, Arc<DeviceINode>>>,
}
...
impl AsyncSimpleFileSystem {
    /// Load SFS from device
    pub async fn open(device: Arc<dyn AsyncDevice>) -> vfs::Result<Arc<Self>> {
    ...
    /// Create a new SFS on blank disk
    pub async fn create(device: Arc<dyn AsyncDevice>, space: usize) -> vfs::Result<Arc<Self>> {
        let blocks = (space + BLKSIZE - 1) / BLKSIZE;
        let freemap_blocks = (space + BLKBITS * BLKSIZE - 1) / BLKBITS / BLKSIZE;
        assert!(blocks >= 16, "space too small");
    ...
    /// Wrap pure AsyncSimpleFileSystem with Arc
    /// Used in constructors
    fn wrap(self) -> Arc<Self> {
        // Create an Arc, make a Weak from it, then put it into the struct.
        // It's a little tricky.
    ...
    /// Create a new INode struct, then insert it to self.inodes
    /// Private used for load or create INode
    fn _new_inode(&self, id: INodeId, disk_inode: Dirty<DiskINode>) -> Arc<AsyncINodeImpl> {
        let device_inode_id = disk_inode.device_inode_id;
        let inode = Arc::new(AsyncINodeImpl {
            id,
            disk_inode: RwLock::new(disk_inode),
            fs: self.self_ptr.upgrade().unwrap(),
            device_inode_id,
    ...
    /// Get inode by id. Load if not in memory.
    /// ** Must ensure it's a valid INode **
    async fn get_inode(&self, id: INodeId) -> Arc<AsyncINodeImpl> {
        assert!(!self.free_map.read()[id]);
    ...
    /// Create a new INode file
    fn new_inode_file(&self) -> vfs::Result<Arc<AsyncINodeImpl>> {
        let id = self.alloc_block().ok_or(FsError::NoDeviceSpace)?;
        let disk_inode = Dirty::new_dirty(DiskINode::new_file());
        Ok(self._new_inode(id, disk_inode))
    }
    /// Create a new INode symlink
    fn new_inode_symlink(&self) -> vfs::Result<Arc<AsyncINodeImpl>> {
        let id = self.alloc_block().ok_or(FsError::NoDeviceSpace)?;
        let disk_inode = Dirty::new_dirty(DiskINode::new_symlink());
        Ok(self._new_inode(id, disk_inode))
    }
    /// Create a new INode dir
    async fn new_inode_dir(&self, parent: INodeId) -> vfs::Result<Arc<AsyncINodeImpl>> {
        let id = self.alloc_block().ok_or(FsError::NoDeviceSpace)?;
        let disk_inode = Dirty::new_dirty(DiskINode::new_dir());
        let inode = self._new_inode(id, disk_inode);
    ...
    /// Create a new INode chardevice
    pub fn new_inode_chardevice(&self, device_inode_id: usize) -> vfs::Result<Arc<AsyncINodeImpl>> {
        let id = self.alloc_block().ok_or(FsError::NoDeviceSpace)?;
        let disk_inode = Dirty::new_dirty(DiskINode::new_chardevice(device_inode_id));
        let new_inode = self._new_inode(id, disk_inode);
        Ok(new_inode)
...
#[async_trait]
impl vfs::AsyncFileSystem for AsyncSimpleFileSystem {
    ...
    async fn root_inode(&self) -> Arc<dyn vfs::AsyncINode> {
...
impl Drop for AsyncSimpleFileSystem {
    /// Auto sync when drop
    fn drop(&mut self) {
        ...
        // unimplemented!();
        // self.sync().await
        //     .expect("Failed to sync when dropping the AsyncSimpleFileSystem");
    }
}
